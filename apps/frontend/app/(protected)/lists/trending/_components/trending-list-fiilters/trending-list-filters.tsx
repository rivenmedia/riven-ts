...
export interface TrendingListFiltersProps {
  ...
  onClear: () => void;
  onApply: (data: TrendingListFiltersFormValues) => void;
  onSortChange: (value: string) => void;
  mediaType: Extract<MediaItemType, "movie" | "show">;
}

export function TrendingListFilters({
  hasActiveFilters,
  onClear,
  onApply,
  onSortChange,
  mediaType,
}: TrendingListFiltersProps) {
  const [isOpen, setIsOpen] = useState(false);

  const sortOptions = useMemo(
    () =>
      SORT_OPTIONS.filter(({ allowedFor }) =>
        (allowedFor as readonly string[]).includes(mediaType),
      ),
    [mediaType],
  );
  ...
  const form = useForm({
    resolver: zodResolver(TrendingListFiltersFormSchema),
  ...
  function createRangeValueUpdater(
    fieldName: keyof TrendingListFiltersFormValues,
  ) {
  ...
  function calculateTotalAppliedFilters() {
    ...
    const groups: [...(keyof TrendingListFiltersFormValues)[]][] = [
      ["runtime"],
      ["voteAverage"],
      ["voteCount"],
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        <SelectTrigger className="h-9 w-45 gap-2">
