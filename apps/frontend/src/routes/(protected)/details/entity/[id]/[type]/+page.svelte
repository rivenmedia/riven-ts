<script lang="ts">
  ...
  const sourceLabel = $derived(
    data.entity.indexer === "tvdb" ? "TVDB" : "TMDB",
  );
  ...
  const movieCredits = $derived(
    data.entity.cast_credits.filter((c) => c.media_type === "movie"),
  );
  const showCredits = $derived(
    data.entity.cast_credits.filter((c) => c.media_type === "tv"),
  );
  ...
  const backdropCandidates = $derived(
    uniqueCredits
      .filter(
        (c: (typeof uniqueCredits)[number]) =>
          c.backdrop_path && c.vote_count != null,
      )
      .sort(
        (
          a: (typeof uniqueCredits)[number],
          b: (typeof uniqueCredits)[number],
...
{#key data.entity.id}
  <div class="relative flex min-h-screen flex-col">
    ...
    {#if birthdayToday && !data.entity.deathday}
      <div class="pointer-events-none fixed inset-0 z-50 overflow-hidden">
        {#each Array(20) as i (i)}
          <div
            class="confetti {CONFETTI_CONFIG.shapes[
              i % CONFETTI_CONFIG.shapes.length
            ]}"
            style="
                left: {(i * 5.26) % 100}%;
                background: {CONFETTI_CONFIG.colors[
              i % CONFETTI_CONFIG.colors.length
            ]};
    ...
    <div class="z-10 flex w-full flex-col">
      ...
      <!-- Credits -->
      <div
        class="mx-auto flex w-full max-w-600 flex-col gap-20 px-8 pb-24 md:px-20 lg:px-24"
      >
  ...
  {#if credits.length > 0}
    <section in:fly|global={{ y: 20, duration: 400, delay, easing: cubicOut }}>
      <div class="mb-6 flex items-baseline gap-3">
        <h2
          class="font-heading text-foreground text-3xl font-bold tracking-tight"
        >
          {title}
        </h2>
        <span class="font-mono text-lg font-medium text-zinc-400"
          >({credits.length})</span
        >
      </div>
