    ...
    return items.map((item) => {
      ...
      if (rawType === "movie") {
        ...
        if (item.episodeNumber != null) {
          detailParams.push(
            `episode=${encodeURIComponent(item.episodeNumber)}`,
          );
        }
      }
...
<PageShell
  class="relative z-10 flex min-h-screen flex-col overflow-x-hidden bg-transparent"
>
  <div class="relative z-10 mx-auto flex w-full max-w-600 flex-col gap-8">
    <!-- Header Section -->
    <header
      class="flex flex-col justify-between gap-6 pt-32 md:flex-row md:items-end md:pt-0"
    >
      <div class="space-y-2">
        <h1
