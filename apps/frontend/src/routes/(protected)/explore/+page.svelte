<script lang="ts">
  ...
  let hasResults = $derived(
    Array.isArray(searchStore.results) && searchStore.results.length > 0,
  );

  // Hero item derived from rotation
  let heroItem = $derived(
    data.heroItems && data.heroItems.length > 0
      ? data.heroItems[currentHeroIndex]
      : null,
  );
  ...
  let backgroundImage = $derived(
    hasResults && searchStore.results[0]
      ? (searchStore.results[0].backdrop_path ??
          searchStore.results[0].poster_path)
      : !hasResults && heroItem
  ...
  onMount(() => {
    ...
    const exampleInterval = setInterval(() => {
      if (data.searchExamples?.length > 0) {
        currentExampleIndex =
          (currentExampleIndex + 6) % data.searchExamples.length;
      }
    }, 4000);
...
<div class="relative min-h-screen w-full overflow-x-hidden">
  ...
  {#if backgroundImage}
    <div
      class="fixed top-0 left-0 z-0 h-screen w-full transition-opacity duration-1000"
    >
      {#key backgroundImage}
        <img
          alt=""
  ...
  {:else}
    ...
    <div class="pointer-events-none fixed inset-0 z-0">
      <div
        class="absolute inset-0 bg-linear-to-b from-zinc-900 via-zinc-950 to-black"
      ></div>
      <div
  ...
  >
    ...
    <div class="flex flex-col gap-4">
      ...
      {#if searchStore.error}
        <div
          class="rounded-lg border border-red-500 bg-red-500/10 p-4 text-red-500"
        >
                  ...
                  <div class="flex items-center gap-4">
                    ...
                    {:then ratings}
                      ...
                      {:else if heroItem.vote_average}
                        ...
                        >
                          <span class="font-black text-[#01b4e4]">TMDB</span>
                          <span class="text-sm font-bold text-white">
