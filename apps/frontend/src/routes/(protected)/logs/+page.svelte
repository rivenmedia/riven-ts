...
{#snippet liveLine(line: LiveLogLine)}
  ...
  >
    <div
      class="text-foreground/90 p-2 font-mono text-xs wrap-break-word whitespace-pre-wrap"
    >
      {line}
    </div>
  </div>
...
{#snippet logEntry(log: LogEntry)}
  ...
  >
    ...
    >
      ...
      <span
        class="shrink-0 font-semibold uppercase {levelColors[level] ??
          'text-foreground'}">{level}</span
      >
...
{#snippet errorDisplay(
  ...
  <div class="bg-destructive/10 border-destructive/20 rounded-lg border p-6">
    <h3 class="text-destructive mb-3 text-lg font-semibold">
      Error Loading Logs
    </h3>
    <pre
...
<PageShell class="h-full">
  ...
  {:else if logs.length > 0 || historicalLogs.length > 0 || connectionStatus !== "disconnected" || isLoadingHistorical}
    <div class="flex h-full min-h-0 flex-col">
      <div
        class="mb-6 flex flex-col items-start justify-between gap-4 md:flex-row"
      >
        <div>
        ...
        <div class="flex items-center gap-4">
          <Button variant="secondary" onclick={handleUploadLogs}
            >Upload Logs</Button
          >
          <div
      ...
      <div
        class="bg-card flex min-h-0 flex-1 flex-col rounded-lg border shadow-sm"
      >
        <div
