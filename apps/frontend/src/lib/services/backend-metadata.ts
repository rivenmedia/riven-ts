...
function mapGqlTmdbListItem(item: GqlTmdbListItem): TMDBTransformedListItem {
  return {
    ...
    ...(item.backdropPath && { backdrop_path: item.backdropPath }),
    ...(item.genreIds && { genre_ids: item.genreIds }),
    ...(item.releaseDate && { release_date: item.releaseDate }),
    ...(item.firstAirDate && { first_air_date: item.firstAirDate }),
    ...(item.originalTitle && { original_title: item.originalTitle }),
    ...(item.originalLanguage && { original_language: item.originalLanguage }),
