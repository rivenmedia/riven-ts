...
export class FilterStore {
  ...
  buildParams(mediaType?: "movie" | "tv"): FilterParams {
    ...
    // Runtime
    if (this.runtimeMin > RUNTIME_CONFIG.min)
      params["with_runtime.gte"] = this.runtimeMin;
    if (this.runtimeMax < RUNTIME_CONFIG.max)
      params["with_runtime.lte"] = this.runtimeMax;
    ...
    // Vote count
    if (this.voteCountMin > VOTE_COUNT_CONFIG.min)
      params["vote_count.gte"] = this.voteCountMin;
    if (this.voteCountMax < VOTE_COUNT_CONFIG.max)
      params["vote_count.lte"] = this.voteCountMax;
