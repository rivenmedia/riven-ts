  ...
  <div class="space-y-6">
    <div>
      ...
      <p class="text-muted-foreground mt-2 text-sm md:text-base">
        Riven ties your media server, scrapers, metadata providers, and request
        services into one workflow so new media can be found, matched, and
        delivered with the defaults you choose.
      </p>
    </div>
    ...
    <Card.Root class="rounded-3xl px-6 py-6">
      <Card.Header class="px-0 py-0">
        <Card.Title class="text-base"
          >This setup will walk you through</Card.Title
        >
      </Card.Header>
    ...
    <p class="text-muted-foreground text-sm">
      Core boot variables like backend, database, and auth are still configured
      outside the app.
    </p>
  </div>
</div>
