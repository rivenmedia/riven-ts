<script lang="ts">
  ...
  function fetchSummary(
    ...
    const rankFields = (tab.fields ?? []).filter(
      (f) => f.type === "custom_rank",
    );
  ...
  function parseDefaultValue(fieldDef: SettingFieldDef): unknown {
    if (fieldDef.default_value == null) {
      ...
      if (fieldDef.type === "object")
        return createDefaultObject(fieldDef.fields ?? []);
  ...
  function updateDictionaryField(
    entryKey: string,
    itemKey: string,
    nextValue: unknown,
  ) {
    const dictionary = ensureDictionary();
    const entry = ensureDictionaryEntry(
      dictionary,
      entryKey,
      field.item_fields ?? [],
    );
...
>
  {#if field.type === "object" && field.display === "tabs"}
    ...
    {@const current =
      tabs.find((t) => t.key === activeTab)?.key ?? tabs[0]?.key ?? ""}
    <div class="space-y-1">
    ...
    <div class="rounded-lg border">
      ...
      {#each tabs as tab (tab.key)}
        {#if tab.key === current}
          {@const tabValue = ensurePlainObject(
            objectValue[tab.key],
            tab.fields ?? [],
          )}
  ...
  {:else if field.type === "custom_rank"}
    ...
    <div
      class="bg-muted/30 flex items-center justify-between gap-3 rounded-lg px-3 py-2"
    >
