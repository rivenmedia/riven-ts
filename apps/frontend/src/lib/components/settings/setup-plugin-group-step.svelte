<script lang="ts">
  ...
  const activeCard = $derived(
    section.plugins.find((view) => view.section.id === activePluginName) ??
      null,
  );
</script>
...
<div class="space-y-8">
  {#if section.plugins.length === 0}
    <div
      class="mx-auto max-w-2xl rounded-3xl border border-dashed p-8 text-center"
    >
  ...
  {:else}
    <div
      class="mx-auto grid w-full max-w-4xl gap-3 sm:grid-cols-2 xl:grid-cols-3"
    >
  ...
  {#if section.plugins.length > 1}
    <div class="mx-auto max-w-2xl">
      <p class="text-muted-foreground text-sm">
        Choose one provider to configure.
      </p>
    </div>
  {/if}
</div>
