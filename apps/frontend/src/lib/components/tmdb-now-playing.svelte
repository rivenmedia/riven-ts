...
{#if Array.isArray(data) && data.length > 0}
  <div
    class="border-border/50 relative overflow-hidden rounded-2xl border shadow-2xl"
  >
    <Carousel.Root
    ...
    >
      <span
        class="font-mono text-xs font-medium whitespace-nowrap text-white/90"
      ...
      <div class="hidden gap-1.5 lg:flex">
        {#each data.map((__, idx) => idx) as i (i)}
          ...
          >
            {#if i === currentIndex}
              <div
                class="animate-progress bg-primary absolute top-0 bottom-0 left-0"
              ></div>
            {/if}
          </button>
        {/each}
...
{:else}
  <div class="relative w-full overflow-hidden rounded-2xl {heightClass}">
    <div
      class="from-background to-muted absolute inset-0 animate-pulse bg-linear-to-t"
    ></div>
