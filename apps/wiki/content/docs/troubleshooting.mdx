...
4. Ensure `RIVEN_SETTING__vfsMountPath` matches the **container-side** path
   (`/mount` above), not the host path.
...
The host is the compose service name, not `localhost` — `localhost` inside the
...
_inside_ the container and publishing the port has no effect. Bind it to all
interfaces:
...
4. Ensure `plexLibraryPath` / `jellyfinLibraryPath` matches the path as the
   **media server** sees it (`/mount` above)
