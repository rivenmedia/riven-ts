 ...
 * ```
 */
export function plexOAuth(
  options: PlexOAuthOptions & { baseURL?: string },
): GenericOAuthConfig {
  if (!options.clientId) {
  ...
  return {
    ...
    getToken: async ({ code }) => {
      ...
      if (!pinStatus.authToken) {
        throw new Error(
          "No auth token in PIN response - user may not have authorized",
        );
      }
...
export const defaultPlexOptions = {
...
} as const satisfies PlexOAuthOptions;
