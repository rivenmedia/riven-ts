...
Don't confuse this with `returnServerError()`: `throwServerError` throws the _handled_ error (post-`handleServerError`) instead of returning it, while `returnServerError(payload)` returns a typed _expected_ error in `result.serverError`, bypassing `handleServerError` entirely (see the safe-action-client skill's error-handling doc).
