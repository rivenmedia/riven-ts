...
3. **On the client**, the hook catches the rejection, sets `status` to `"hasNavigated"`, and fires `onNavigation` / `onSettled` with the `navigationKind`. It does _not_ re-throw by default — `result` is the empty branch, and `navigationKind` is a callback argument, not a field on `SafeActionResult`
