...
export default {
  ...
  hooks: {
    ...
    "riven.media-item.scrape.requested": async ({ dataSources, event }) => {
      ...
      // only `Record<infoHash, rawTitle>`. The captured size / seeders /
      // leechers fields are intentionally dropped here until the contract is
      // widened in a follow-up commit (see the migration introducing
      // `stream.size` / `stream.seeders` / `stream.leechers`).
